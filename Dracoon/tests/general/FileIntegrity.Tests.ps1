...
Describe "Verifying integrity of module files" {
	...
	Context "Validating PS1 Script files" {
		$allFiles = Get-ChildItem -Path $moduleRoot -Recurse | Where-Object FullName -NotLike "$moduleRoot\internal\autorest\*" | Where-Object Name -like "*.ps1" | Where-Object FullName -NotLike "$moduleRoot\tests\*"
