Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\Dracoon.psd1") -join "`n") | Invoke-Expression
	Context "Basic resources validation" {
		$files = Get-ChildItem "$moduleRoot\functions" -Recurse -File | Where-Object Name -like "*.ps1"
		$files += Get-ChildItem "$moduleRoot\internal\autorest\out" -Recurse -File | Where-Object Name -like "*.ps1"
