...
$parentModule = 'Dracoon'
if (-not $ModuleName) { $ModuleName = 'Dracoon.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Dracoon'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Dracoon.psd1"
$prereqHash = @{
