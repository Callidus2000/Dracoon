...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'Dracoon.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'Dracoon.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'Dracoon.Client.ProtectedToken'
